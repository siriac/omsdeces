  ...
  function getDayClass(data) {
	...
	$scope.submitForm = function(){
		...
		$http.post(url, data, config).then(function (response) {
			alert(response.data.region);
			dataShare.sendData(response.data);
			alert("after datashare");
			$location.url("/Confirmation.html");
...
app.controller('confirmationCTRL', function($scope, $http, $location,dataShare) {
	$scope.$on('data_shared',function(){
	      var data = dataShare.getData();
	      alert(data);
	});
	$scope.postResultMessage = dataShare.getData();
	alert("bonjour confirmation1 "+" "+$scope.postResultMessage);
});

app.controller('loginCTRL',function($scope, $http,$window,$location) {
	alert("i m here in login");
	$scope.username;
	$scope.password;
	$scope.submitForm = function(){
		alert("dans fonction");
		...
		.then(function(response){
			$scope.persons = response.data;
			$window.location.href = '/Annexe2.html';
			$location.url('/Annexe2.html');
			$scope.Message="Authentification avec succes"
				alert("Vous etes authentifie en tant que "+response.data.userName);
		});
};
});
